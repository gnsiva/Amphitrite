...
class MassSpectrum(tdd.TwoDdata):
    ...
    def plotPeakWidthLimits(self,name,charge,leftMultiplier=1,rightMultiplier=1,ax=False):
        """Fill in the area between the max and min limits of FWHM around the
        peak mean. (only use after calculating the mass spectrum fit).
        ...
        :parameter ax: Matplotlib Axes object or False to create one
        :returns: m/z limits when using multiplier as [lowerLimit,upperLimit]
        """
        ax = utils.checkAx(ax)
        lims = self.getPeakWidthLimits(name,charge,leftMultiplier,rightMultiplier)
        ylims = ax.get_ylim()
        
        ax.fill_betweenx(ylims,lims[0],lims[1],color='red',alpha=0.3)
